...
# Причина, по которой сервис не поднялся, — у него же в журнале. Контейнер с
# `restart: unless-stopped`, упавший на разборе настроек, перезапускается вечно,
# и `docker compose ps` говорит про него только «Restarting (1)»: это читается
# как «что-то с контейнером», хотя сервер уже назвал причину одной внятной
# строкой («[mcp] не поднялся: ...»). Смоук её показывает — иначе человек идёт
# за ней в логи руками, а там она повторена столько раз, сколько было
# перезапусков. Берётся ПОСЛЕДНЯЯ непустая строка: у отказа настроек она одна и
# та же на каждом круге.
# ИМЕНА ЗДЕСЬ ЛАТИНИЦЕЙ, и это не стиль: кириллическое имя переменной bash не
# принимает вовсе, а `bash -n` синтаксис считает целым — скрипт падает уже на
# прогоне. Поймано в этом файле дважды.
cause() {
    last=$(remote "docker compose logs --tail=20 $1 2>&1 | Select-Object -Last 5" \
        | grep -v '^[[:space:]]*$' | tail -1)
...
await_health() {
    ...
    while :; do
        ...
        case "$status" in
            ...
            *unhealthy*) say "$label" "ПРОВАЛ (нездоров: $status)$(cause "$svc")"; fail=1; return ;;
            *healthy*) say "$label" "OK ($status)"; return ;;
            # ВНИМАНИЕ, порядок веток, ровно как с «unhealthy» выше: строка
            # «Restarting» СОДЕРЖИТ в себе «starting», и ветка ожидания,
            # стоящая первой, читает вечный перезапуск как «ещё поднимается» —
            # то есть смоук молча ждёт HEALTH_WAIT секунд и не называет
            # причину, хотя она уже лежит в журнале сервиса.
            *Restarting*)
                say "$label" "ПРОВАЛ (перезапускается: $status)$(cause "$svc")"; fail=1; return ;;
            *starting*)
            ...
            *) say "$label" "ПРОВАЛ ($status)$(cause "$svc")"; fail=1; return ;;
        esac
    done
}
...
# С #216 звено — СЕРВИС стенда (`link`, профиль `funnel`), а не контейнер,
# поднятый руками. Поэтому и спрашивается оно как сервис, наравне с ботом и
# сервером. Пока смоук искал ручной контейнер, правильно собранный стенд
# краснел, и человек в ответ поднимал звено руками — то есть проверка
# возвращала ровно то, что задача убирала.
link_status=$(service_status link)
case "$link_status" in
    "") say "звено до сервера" "ПРОВАЛ: сервиса link нет в стенде — забыт COMPOSE_PROFILES=funnel"; fail=1 ;;
    Up*) say "звено до сервера" "OK ($link_status)" ;;
    *) say "звено до сервера" "ПРОВАЛ ($link_status)$(cause link)"; fail=1 ;;
esac

# Ручные копии звена, оставшиеся от прежнего порядка. Каждая поднята с
# автоперезапуском и сидит в сетевом пространстве УЖЕ УДАЛЁННОГО сервера:
# наружу не отдаёт ничего и при этом выглядит в списке рабочей. Дважды за
# неделю такая копия подменила диагноз — сначала «звено не поднято», потом
# «работает звено», хотя работало не оно. Пока она жива, ответа на вопрос
# «через что идёт доступ» нет, поэтому её наличие — провал, а не примечание.
orphan=$(remote "docker ps --filter name=dodo-mcp-proxy --format '{{.Names}} {{.Status}}'")
if [ -z "$orphan" ]; then
    say "ручных копий звена нет" "OK"
else
    say "ручных копий звена нет" "ПРОВАЛ: рядом живёт ручная копия ($orphan) — снять docker stop"
    fail=1
fi
