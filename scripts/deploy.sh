...
#
# `--profile backup` — не украшение: голая `docker compose build` собирает
# ТОЛЬКО сервисы активных профилей, то есть выгрузку состояния (#233) молча
# пропускает. Проверено `--dry-run`: без профиля в сборке два образа из трёх.
# Итог был бы такой: раскатка прошла, а ночная задача бэкапа крутит образ
# прошлой версии — или не находит его вовсе и лезет собирать сама, посреди
# расписания. Профиль здесь только включает сервис в СБОРКУ; поднимать его
# `up -d` ниже по-прежнему не будет, профиля в `.env` площадки нет.
step "сборка образа с версией $sha"
remote "\$env:BUILD_SHA='$sha'; docker compose --profile backup build" | tail -2 || exit 1
