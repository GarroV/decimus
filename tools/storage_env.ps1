...
foreach ($name in $needed.Keys) {
    ...
    if ($null -eq $value) {
        ...
        #
        # Источник случайности — КРИПТОГРАФИЧЕСКИЙ, и это не педантизм.
        # `Get-Random` тянет из обычного генератора псевдослучайных чисел: его
        # выход восстанавливается по зерну, а зерно на деле угадывается. Этим
        # паролем закрыты доказательства проверок, по которым партнёр получает
        # оценку, — цена угаданного пароля тут не «переустановить сервис».
        ...
        # Остаток от деления даёт едва заметный перекос к началу алфавита (256
        # не делится на 62 нацело). На 40 знаках из 62-буквенного алфавита это
        # не приближает перебор ни на сколько значимое, а отбраковка лишних
        # байтов усложнила бы скрипт ради ничего.
