...
#: Цели, которые гоняют набор тестов. Доступ к базе у них обязан быть один и тот
#: же: разошёлся — часть прогонов идёт без базы и снимает её тесты молча (#250).
ЦЕЛИ_ПРОГОНА = ("test", "test-honest")


def _доступы_цели(цель: str) -> set[str]:
    """Имена переменных, которые Makefile отдаёт дочернему процессу этой цели."""
    образец = re.compile(rf"^{re.escape(цель)}: export ([A-Z_]+)\s*=", re.M)
    return set(образец.findall(MAKEFILE.read_text(encoding="utf-8")))


def test_честный_прогон_получает_ту_же_базу_что_обычный() -> None:
    # Дефект #250: у `test-honest` не было ни одного `export`, поэтому «честный»
    # прогон шёл без DATABASE_URL и снимал все тесты блока db, оставаясь зелёным
    # с нулём падений. Хуже обычного пропуска: цель, само имя которой обещает
    # честность, показывала зелёный на непроверенном коде.
    обычный, честный = (_доступы_цели(цель) for цель in ЦЕЛИ_ПРОГОНА)
    assert обычный, "у цели test пропал доступ к базе — сравнивать стало не с чем"
    assert честный == обычный, (
        "цели прогона разошлись по доступам к базе: "
        f"только у test — {sorted(обычный - честный)}, "
        f"только у test-honest — {sorted(честный - обычный)}"
    )
