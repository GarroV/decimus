...
#: Подключение администратора нужно каждому тесту файла: правка карты без него
#: не идёт вовсе. Поэтому фикстура объявлена разом на файл, а не аргументом в
#: два десятка подписей, — тот же приём, что у `usefixtures` в тестах бота.
pytestmark = [requires_db, pytest.mark.usefixtures("curation_env")]
...
def test_снятый_синоним_остаётся_строкой_с_причиной() -> None:
...
def test_снятый_синоним_не_находится_поиском() -> None:
...
def test_снятого_синонима_нет_в_карте_а_с_признанием_он_виден() -> None:
...
def test_повторное_снятие_причину_не_переписывает() -> None:
...
def test_причина_снятия_обязательна() -> None:
...
def test_заведённое_человеком_и_машиной_снимается_одинаково(происхождение: str) -> None:
...
def test_снятую_формулировку_машина_заново_не_выучивает() -> None:
...
def test_снятую_формулировку_не_выучивает_и_другой_пункт() -> None:
...
def test_синоним_переправляется_на_другой_пункт() -> None:
...
def test_правка_оставляет_видимым_куда_строка_вела_раньше() -> None:
...
def test_правка_на_тот_же_пункт_ничего_не_меняет() -> None:
...
def test_причина_правки_обязательна() -> None:
...
def test_правка_возвращает_снятую_строку_в_работу() -> None:
...
def test_возврат_снятой_строки_с_тем_же_пунктом() -> None:
...
def test_незнакомую_формулировку_снять_нельзя() -> None:
...
def test_незнакомую_формулировку_переправить_нельзя() -> None:
...
def test_чужого_арендатора_правка_не_достаёт() -> None:
...
def test_правка_находит_строку_по_другому_написанию() -> None:
...
def test_пустая_формулировка_ничего_не_снимает() -> None:
...
def test_правка_без_подключения_администратора_отказывает(
    ...
    ответы.
    """
    # Переменную снимает сам тест: фикстура файла ставит её каждому, и здесь
    # проверяется ровно то, чего без неё не бывает.
