...
def _проверка_с_находками(
    ...
    *,
    ...
    Код взят МНОГОЗОННЫЙ намеренно. С T271 движок отвергает пару «пункт + зона»,
...
def test_source_пусто_без_записи_и_comment_со_слов_аудитора(domain_env: Path, db_env: str) -> None:
    ...
    add_finding(chat_id, code="CLN03", level="D1", zone="hot_kitchen", text="без источника")
    add_finding(
        chat_id,
        code="CLN03",
        level="D1",
