...
Отдельно проверяется форма ссылки. Хранилище на этом проекте заведомо переедет
(D054, D061, D166), и вся дешевизна переезда держится ровно на одном: в ссылке
нет ни адреса сервера, ни имени поставщика. Ссылка, знающая сегодняшний адрес,
превратила бы переезд в правку каждой строки истории — поэтому это требование,
а не наблюдение.
"""
...
def test_ссылка_не_знает_адреса_сервера_иначе_переезд_хранилища_станет_правкой_истории(
    domain_env: Path, db_env: str
) -> None:
    inspection_id = _проверка(910_002)

    сохранённый = upload_report(inspection_id, data=ОТЧЁТ, storage=ЗаписнойСклад())

    assert сохранённый.storage_path.startswith("s3://")
    # Ни адреса, ни поставщика: на этом стоит обещание, что переезд хранилища
    # стоит правки переменных окружения, а не правки каждой строки истории.
    for запрещённое in ("http://", "https://", "storage-live", "minio", ":9000"):
        assert запрещённое not in сохранённый.storage_path
