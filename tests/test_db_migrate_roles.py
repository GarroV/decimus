...
APP_TABLE_GRANTS: dict[str, set[str]] = {
    ...
    # Зафиксированное письмо тоже не переписывается (`0019`): из истории
    # нельзя вынуть отправленное, иначе она перестаёт быть историей.
    "partner_letters": {"SELECT", "INSERT"},
    ...
    # Учётки админки роль приложения ТОЛЬКО ЧИТАЕТ (`0014`, T323): завести,
    # сменить чужой пароль или отключить учётку она не может — это отказ базы,
    # а не отсутствие кода. Появится здесь INSERT или UPDATE — получив базу
    # продукта, чужой заведёт себе вход, и поймать это, кроме как тут, негде.
    "web_users": {"SELECT"},
    # Сессия открывается и читается; правится ровно одна колонка — ниже.
    "web_sessions": {"SELECT", "INSERT"},
    # Счётчик неудачных входов (`0015`, T325) — рабочая запись самой роли
    # приложения, а не чужая тайна: она его и заводит, и обнуляет при удачном
    # входе, и чистит от старых строк. DELETE здесь поэтому законен, в отличие
    # от `mcp_tokens` и `web_users`: таблица целиком восстанавливается
    # следующей же неудачной попыткой, а сведений о том, кто здесь заведён, в
    # ней не лежит вовсе — только отпечатки.
    "web_login_attempts": {"SELECT", "INSERT", "DELETE"},
...
APP_COLUMN_GRANTS: dict[str, dict[str, set[str]]] = {
    ...
    # `expires_at` или переписать `fingerprint` на свой, то есть присвоить
    # чужой вход. Сужающая политика говорит то же про односторонность, но
    # только про UPDATE закрытия; неприкосновенность остальных колонок держит
    # ровно эта строка.
    "web_sessions": {"UPDATE": {"closed_at"}},
    # Правятся счётчик и время, и только они (`0015`). Станет грант табличным —
    # и накопленные неудачи можно будет переставить с чужого ключа на свой
...
ADMIN_TABLE_GRANTS: dict[str, set[str]] = {
    ...
    # Счётчик неудачных входов (`0016`, T328, D158): читать — чтобы показать,
    # кто заперт; удалять — это и есть снятие запрета, потому что срок в базе
    # не хранится, а считается по числу неудач. INSERT и UPDATE не выданы
    # намеренно: роль, которая может ДОБАВИТЬ неудачи чужому логину, умеет
    # запирать вход человеку — то есть делать ровно ту беду, от которой команда
    # снятия и заведена.
    "web_login_attempts": {"SELECT", "DELETE"},
}
...
ADMIN_COLUMN_GRANTS: dict[str, dict[str, set[str]]] = {
    ...
    # печатала бы столбец шестнадцатеричных строк. Станет этот грант табличным
    # — и роль, которой нужны логины, получит заодно свёртки паролей.
    # `0020` (T338) добавил сюда роль и дату заведения: экран управления
    # учётками показывает, кто заведён, когда и с какими правами. Пароля в
