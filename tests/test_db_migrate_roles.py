...
APP_TABLE_GRANTS: dict[str, set[str]] = {
    ...
    # История переносов (`0025`, D195) — только чтение. Пишет её триггер от
...
ADMIN_COLUMN_GRANTS: dict[str, dict[str, set[str]]] = {
    ...
    # `0021` (T340) добавил почту: по ней зовут человека в письме и по ней же
    # его узнают, поэтому она читается, пишется и правится. `0022` (T343) дал
    # UPDATE на свёртку пароля — это смена пароля учётке, и грант колоночный
    # ровно затем, чтобы роль не получила права переписать логин и арендатора
    # заодно. В SELECT свёртки по-прежнему нет: менять пароль можно, не читая
    # старый, и читать его незачем никому.
    "web_users": {
        "SELECT": {"id", "login", "tenant_code", "disabled_at", "role", "created_at", "email"},
        "INSERT": {"tenant_code", "login", "password_hash", "role", "email"},
        "UPDATE": {"disabled_at", "role", "email", "password_hash"},
    },
    # Смена пароля закрывает открытые сессии учётки (`0022`): иначе пароль
    # сменили, а старый вход продолжает работать — и человек, ради которого
    # пароль меняли, остаётся внутри. Отсюда UPDATE ровно на дату закрытия.
    "web_sessions": {
