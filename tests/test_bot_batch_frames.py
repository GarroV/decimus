...
Дословная формулировка владельца лежит в `docs/furca/decisions.md`, D081; сюда
...
async def test_кадры_с_комментарием_разбираются_вместе_и_остаются_одной_записью(
    ...
    """Пачка с подписью уходит в модель со всеми кадрами (D208), запись — одна.

    Сказанное человеком слово — по-прежнему одно нарушение с несколькими фото
    (`docs/06-mvp-bot.md`, шаг 3). Изменилось другое: до D208 кадры с подписью в
...
async def test_увиденное_на_кадрах_только_предлагается(
    ...
    """Увиденное сверх слов показывается отдельным сообщением и само не пишется (D208)."""
    started()
    ответ = Suggestion(
