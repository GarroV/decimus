...
def test_edit_смены_кода_на_зону_вне_методики_отклоняется(
    started: Callable[..., Run], workdir: Path
) -> None:
    """CLN05 применим только к горячему цеху: перевод на него из зала методика не даёт (T271, #239).

    До задачи T271 такая правка проходила и лишь помечала запись флагом
    `zone_unusual` — предупреждением, которое видел только тот, кто смотрел
    вывод команды. Теперь движок пару «пункт + зона» вне методики отклоняет
    вовсе: смена кода пункта не должна тайком протащить зону, которую новый
    пункт не держит.
    """
    started("add", "--qid", "CLN06", "--level", "D1", "--zone", "dining")
    r = started("edit", "--n", "1", "--qid", "CLN05")
    assert r.code != 0, "движок принял пару, которой методика не даёт"
    assert "hot_kitchen" in r.text, f"в отказе нет допустимой для CLN05 зоны: {r.text!r}"
    assert finding(workdir, 1)["qid"] == "CLN06", "код записи сменился вопреки отказу движка"
    assert finding(workdir, 1).get("zone_unusual") is not True, (
        "флаг нетипичности зоны больше не выставляется"
    )
