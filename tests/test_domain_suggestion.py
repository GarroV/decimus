...
Решение D077 говорит о пополнении списка терминов при несостыковках или новой
формулировке аудитора. Пополняет список человек — управляющая компания (D077 запрещает
...
def test_удалённая_запись_уносит_своё_предложение(domain_env: Path) -> None:
    """Снятая запись не оставляет за собой предложение системы.

    Довод «иначе следующая запись с тем же номером унаследует чужое» был
    неверен (T221): движок номера не переиспользует, и новая запись получает
    следующий номер, а не номер снятой. Проверка этого стоит здесь же, потому
    что без неё сравнение предложения у новой записи не могло бы покраснеть
    вовсе — оно проходило бы и на реализации, которая ничего не убирает.

    Настоящий довод: предложение к записи, которой в проверке больше нет, —
    мусор внутри самой проверки, а она едет дальше одним объектом.
    """
    начата()
    add_finding(
        CHAT,
    ...
    drop_finding(CHAT, 1)
    новая = add_finding(CHAT, "CLN05", "D1", "hot_kitchen", "нагар", source=SOURCE_COMMENT)

    state = get_state(CHAT)
    assert state is not None
    (запись,) = state.findings
    assert новая.n != 1, "номер снятой записи выдан заново — сквозная нумерация нарушена"
    assert запись.suggested_code == "", "новая запись получила предложение удалённой"
    assert "1" not in блок().get("suggestions", {}), "предложение снятой записи осталось в проверке"
