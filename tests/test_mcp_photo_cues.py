...
# --- вставка попадает в НАЗВАННЫЙ раздел (T291) --------------------------------


#: Раздел, у которого шапка есть, а строк ещё нет. На сегодняшней карте таких
#: нет — потому дефект и не стрелял, — но он появляется ровно тогда, когда
#: управляющая компания заводит раздел и начинает его наполнять.
ПУСТОЙ_РАЗДЕЛ = "Приёмка"


def _с_пустым_разделом(store: Store) -> None:
    """Дописать в живую карту раздел с шапкой таблицы и без единой строки."""
    файл = store.live / CUES_FILE
    файл.write_text(
...
def test_строка_пустого_раздела_ложится_в_него_а_не_после_первой_строки_файла(
    store: Store,
) -> None:
    """Раздел назван верно, отказа нет — и строка уезжает в чужую таблицу.

    Молчаливая вставка не в то место хуже отказа: раздел в вызове назван, ответ
    говорит «заведено», а карта продукта поднимает эту строку под чужим
    заголовком — то есть с чужими колонками, где «грязь» и «поломка» значат
    разное.
    """
    _с_пустым_разделом(store)

    итог = photo_cues.add(
        store,
        tenant=АРЕНДАТОР,
        section=ПУСТОЙ_РАЗДЕЛ,
        phrase="Коробка вскрыта",
        codes=["CLN01"],
        version_name="imf",
    )
    ...
    assert итог.accepted is True, итог.refusal
    разделы = {
        раздел["section"]: [строка["phrase"] for строка in раздел["cues"]]
        for раздел in photo_cues.read(store, version=итог.version)["sections"]
    }
    assert разделы.get(ПУСТОЙ_РАЗДЕЛ) == ["Коробка вскрыта"], (
        f"строка не попала в названный раздел: {разделы}"
    )
    assert "Коробка вскрыта" not in разделы[РАЗДЕЛ]


def test_первая_строка_карты_без_единой_строки_заводится_а_не_падает(store: Store) -> None:
    """Карта, в которой ещё нет ни одной строки, — законное начало, а не беда.

    Опереться в этом случае не на что вовсе: прежний расчёт брал первую строку
    файла, а её нет, и правка кончалась отказом машины (`IndexError`) вместо
    заведённой строки.
    """
    (store.live / CUES_FILE).write_text(
        f"# Что искать на фото\n\n## {РАЗДЕЛ}\n\n| Что видно | Кандидаты |\n|---|---|\n",
        encoding="utf-8",
    )
