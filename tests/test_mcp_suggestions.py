...
формулировке аудитора — пополняет его управляющая компания, не бот, и это
ровно та граница, которую проверяет этот файл: сборка ГОТОВИТ
...
#: Раздел «Чистота» — одна колонка кандидатов, раздел «Оборудование» — три:
#: «грязь», «поломка» и колонка управляющей компании «Откуда», кодов не
#: несущая вовсе. Это не для красоты: правка принимает ровно те ячейки,
#: которые называет карта, поэтому предложение обязано попадать в ту колонку,
#: где стоит промахнувшийся код, а чужую колонку переписывать как есть.
КАРТА = (
    CueRow(section="Чистота", phrase="Пол в разводах, лужа на полу", cells=("CLN01",)),
    CueRow(section="Чистота", phrase="Стена в подтёках", cells=("CLN02",)),
    CueRow(section="Оборудование", phrase="Печь", cells=("EQP07", "TEH05", "УК")),
)
...
def test_код_дописывается_в_ту_колонку_где_стоит_промахнувшийся() -> None:
    ...
    поломке, а ячеек в вызове обязано быть ровно столько, сколько их называет
    правка, — иначе её отклонят по ширине строки (`photo_cues._check_codes`).

    Колонка управляющей компании при этом переписывается как есть: собранное
    из одних кодов предложение стёрло бы её содержимое (T289)."""
    итог = собрать(находка(suggested_code="TEH05", code="TEH06"))

    assert итог["code_misses"][0]["suggested_edits"] == [
        {
            "tool": "edit_photo_cue",
            "arguments": {"phrase": "Печь", "codes": ["EQP07", "TEH05, TEH06", "УК"]},
        }
    ]
...
def test_промах_называет_фразу_аудитора_а_не_только_строку_карты() -> None:
    """Требование владельца (D077) было про ФРАЗУ, а не только про строку
    карты. Строка карты при этом никуда не девается: управляющей компании
