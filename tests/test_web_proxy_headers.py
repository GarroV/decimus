...
def настройки(*, звенья: int, префикс: str = "") -> Settings:
    return Settings(
        host="127.0.0.1",
...
def приложение_эхо(окружение: dict[str, Any], ответ: Any) -> list[bytes]:
    ...
    корень = окружение.get("SCRIPT_NAME", "")
    ответ("200 OK", [("Content-Type", "text/plain; charset=utf-8")])
    return [f"{схема}|{цепочка}|{корень}".encode()]
...
def test_путь_общего_входа_доезжает_корнем_приложения(поднятый_сервер: Any) -> None:
    """Префикс становится `SCRIPT_NAME`, а значит и корнем всех ссылок.

    Проверяется настоящим сервером, а не разбором настройки: `url_prefix`
    исполняет `waitress`, и «настройка прочитана» ничего не говорит о том,
    доехала ли она до приложения. Ровно на этом различии уже погорела
    `WEB_TRUSTED_PROXIES` (#306).
    """
    адрес = поднятый_сервер(настройки(звенья=0, префикс="/audit"))
