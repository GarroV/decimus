...
# --- 2. Зона вне методики — отказ, а не предупреждение (T271, #239) --------


def test_зона_вне_методики_отклоняется_а_не_помечается(
    started: Callable[..., Run], workdir: Path
) -> None:
    """CLN05 объявлен только для hot_kitchen — до T271 зал проходил с пометкой.

    Живой прогон владельца показал цену тихого предупреждения: пункт про печь
    лёг в холодный цех, `zone_unusual` в выводе никто не читал, и вычет уехал
    партнёру в чужую зону. Теперь пара «пункт + зона» вне методики не проходит
    вовсе, а отказ называет допустимые зоны.
    """
    r = started("add", "--qid", "CLN05", "--level", "D1", "--zone", "dining")
    assert r.code != 0, "движок принял пару, которой методика не даёт"
    assert "hot_kitchen" in r.text, f"отказ не назвал допустимую зону: {r.text!r}"
    assert numbers(workdir) == [], "нетипичная запись всё же попала в состояние"
