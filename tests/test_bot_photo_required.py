...
Владелец пересмотрел вчерашний собственный ответ: сутки назад некоторые
пункты можно было пустить без фотофиксации (D071), сегодня — фотофиксация
обязательна всегда, а бот обязан дать человеку это понять (D078).
