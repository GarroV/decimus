...
def test_нетипичная_зона_отклонена_а_не_помечена_флагом(domain_env: Path) -> None:
    """Движок отказывает: CLN05 заведён только для горячего цеха (T271, #239).

    До задачи T271 движок такую пару ПРИНИМАЛ и лишь помечал флагом
    `zone_unusual` — предупреждением, которое читал только тот, кто смотрел
    вывод команды или базу. Живой прогон владельца показал цену этого: пункт
    про печь лёг в холодный цех, предупреждение проехало мимо, и вычет уехал
    партнёру в чужую зону. Теперь пара «пункт + зона», которой методика не
    даёт, не проходит вовсе.
    """
    начать()
    ...
    with pytest.raises(EngineError) as e:
        add_finding(42, code="CLN05", level="D1", zone="dining", text="нагар в зале")
    сообщение = str(e.value)
    assert "CLN05" in сообщение and "hot_kitchen" in сообщение, (
        f"отказ не назвал ни пункт, ни допустимую для него зону: {e.value}"
    )
    состояние = get_state(42)
    assert состояние is not None and len(состояние.findings) == 1, (
        "отказ движка не помешал нетипичной записи попасть в состояние"
    )


def test_старый_флаг_нетипичной_зоны_дочитывается(domain_env: Path) -> None:
    """Пометка `zone_unusual`, выставленная движком до T271, читается и сейчас (#239).

    Флаг больше не выставляется (`test_нетипичная_зона_отклонена_а_не_помечена_флагом`),
    но выгрузки прошлых лет несут его в состоянии, и `Finding.zone_unusual`
    обязан прочитать её ровно так, как видел движок тогда, — иначе продукт
    молчал бы о находке, которую сам же когда-то и сделал.
    """
    начать()
    add_finding(42, code="CLN05", level="D1", zone="hot_kitchen", text="нагар")
    сырое = json.loads(файл_состояния(domain_env).read_text(encoding="utf-8"))
    сырое["findings"][0]["zone_unusual"] = True
    файл_состояния(domain_env).write_text(json.dumps(сырое, ensure_ascii=False), encoding="utf-8")

    прочитанная = запись(42, 1)
    assert прочитанная is not None and прочитанная.zone_unusual is True, (
        "старую пометку из выгрузки прошлых лет продукт перестал читать"
    )
...
def test_удаление_убирает_запись_и_номер_не_переиспользуется(domain_env: Path) -> None:
    ...
    следующая = add_finding(42, code="CLN06", level="D1", zone="dough", text="c")
