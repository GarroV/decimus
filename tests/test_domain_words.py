...
def test_правка_записи_слова_не_трогает(domain_env: Path) -> None:
    ...
    Код взят CLN06, а не CLN05: методика держит CLN06 и в `hot_kitchen`, и в
    `dining` (T271 отклоняет пару, которой методика не даёт), а тесту нужна
    именно правка зоны на другую валидную.
    """
    _начатая()
    add_finding(CHAT, "CLN06", "D1", "hot_kitchen", "Нагар на подине печи", words=СКАЗАНО)
