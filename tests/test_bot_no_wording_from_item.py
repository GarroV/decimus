...
Вопросы методики сформулированы как утверждение нормы («… делается согласно
стандарту»; дословно не приводим — методика заказчика не цитируется в публичном
репозитории, это сторожит `tests/test_methodology_leak.py`). Ручной выбор пункта
по кадру без комментария клал этот текст в запись как слова аудитора — и партнёр читал в
