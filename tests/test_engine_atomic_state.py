...
def test_параллельные_записи_не_затирают_друг_друга(
    ...
    Список зон — ровно те, что методика держит за PRD09 (T271 отклоняет пару
    вне этого перечня): `facade`, `dishwashing` и `staff` в нём не значились и
    были лишними ещё до задачи, просто движок раньше не отказывал на них.
    """
    zones = [
        "hot_kitchen",
        "cold_kitchen",
