...
`docs/furca/plan.md`, раздел «Архитектура»). Импортировать его в тесты нельзя:
...
#: Базы рядом нет. Отдельный флаг, а не просто `not BASE_DSN` по месту: его
#: читает сводка прогона, и без него отсутствие базы было неотличимо от
#: «нечего проверять» — 339 тестов пропускались молча, а гейт оставался
#: зелёным (задача #207).
NO_DB = not BASE_DSN
...
#: Подключения ПОВЫШЕННЫХ ПОЛНОМОЧИЙ: владелец схемы и администратор снятия.
#: Снимаются вместе с `DATABASE_URL` — и это не симметрия ради красоты. Их
#: выдаёт `.env`, а `.env` попадает в прогон целиком при первом же импорте
#: точки входа (`load_dotenv` там стоит на уровне модуля и срабатывает уже на
#: СБОРЕ тестов, до первого теста). Дальше `web_access._managing` берёт первым
#: `DATABASE_RETRACTION_URL` — и заведение учёток уходило в базу стенда вместо
#: одноразовой. Так набор `test_db_web_access.py` падал 39 тестами в общем
#: прогоне и проходил в одиночку (#356), а до падения успевал ПИСАТЬ в чужую
#: базу.
ПОВЫШЕННЫЕ_ПОДКЛЮЧЕНИЯ = ("DATABASE_ADMIN_URL", "DATABASE_RETRACTION_URL")
...
def _база_не_видна_сама_собой(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for имя in ПОВЫШЕННЫЕ_ПОДКЛЮЧЕНИЯ:
...
def pytest_terminal_summary(terminalreporter: pytest.TerminalReporter) -> None:
    ...
    Без этого отсутствие `data/`, `examples/` или поднятой базы выглядит как
    обычный зелёный прогон: тесты не падают, они пропускаются, и «N passed»
    ничем не отличается от полного набора. На стройке это уже стоило блоку
    исчезнувшего регрессионного якоря — он не заметил, что копия завелась без
    эталонов (задача #175), а позже — 339 тестов блока `db` и MCP, которые
    пропускались на каждом прогоне, пока гейт считался зелёным (задача #207).
    """
    if not (NO_DATA or NO_EXAMPLES or NO_DB):
        return
    чего_нет = [
        имя
        for имя, нет in (
