...
КАРТА_СЛОВ = (
    ...
    # Раздел ровно той формы, что у боевой карты после T262: колонка «Зона»
    # (её продукт читает ПО ЗАГОЛОВКУ) и колонка управляющей компании
    # «Откуда», которая кодов не несёт вовсе. Обе ломали правку карты через
    # агента, пока ширину строки считали числом колонок (T289).
    "\n## Оборудование\n"
    "\n| Объект | Грязь | Поломка | Зона | Откуда |\n|---|---|---|---|---|\n"
    "| Печь | CLN01 | CLN02 | fridge | вопрос |\n"
    "| Стеллаж | CLN01 | — |  | УК |\n"
)
