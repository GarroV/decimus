...
Решение D063 отдало предпочтение сверке со списком нарушений перед
рассуждением модели. Замер это подтверждает наполовину: ждём мы не картинку, а
...
def _vocabulary_now(chat_id: int | None) -> dict[str, frozenset[str]]:
    ...
    молча отключать различение грязи и поломки — карта дополняет встроенный
    минимум, а не заменяет его (D077). Карта читается на каждом
...
def fast_path(
    ...
    **Зона участвует в решении, а не проверяется последней (T143).** Она
    ...
    Замер объясняет, почему именно так: на боевых записях отбрасывание строк по
    зоне даёт одно лишнее срабатывание из семнадцати, и оно неверное.

    **`zone_fixed` — зона взята из словаря объектов самой карты (T262, T265).**
    Тогда отказа `WRONG_ZONE` быть не может по построению: зону назвала та же
    таблица, из которой взяты коды, и расхождение между ними означает не чужую
    подсказку, а противоречие внутри методики — колонка «Зона» карты против
    колонки `zones` чек-листа. Такое противоречие называется в журнале поимённо
    и отказом всё-таки кончается: запись быстрого пути ложится БЕЗ
    подтверждения аудитора (D064), а движок пару «пункт + зона», которой
    методика не даёт, с T271 не принимает вовсе. Молчаливый отказ на этом месте
    был бы худшим из исходов — он выглядел бы как «слова непонятны».

    До T264 сюда приезжала зона ПРОШЛОЙ записи, и `WRONG_ZONE` был обычным
    делом: пункт про печь не проходил из холодного цеха. Источников зоны теперь
    три, памяти среди них нет, и этот отказ остаётся только на настоящем
    расхождении.
    ...
    **`chat_id` обязателен и умолчания не имеет (T226).** Пункты, классы, зоны
    ...
    for cue in covered:
        ...
        if not applicable:
            if zone_fixed:
                logger.warning(
                    "карта кадров и чек-лист расходятся: строка «%s» держит зону «%s», "
                    "а пункты %s в этой зоне методика не даёт",
                    cue.phrase,
                    zone_hint,
                    ", ".join(codes),
                )
