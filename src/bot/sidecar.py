...
def handed_over(chat_id: int) -> bool:
    ...
    владельца D080 эту развилку убрало: сданные отчёты не правятся, и
    дописывать в сданную проверку больше нельзя ничем (`src/bot/sealed.py`).
    Числу теперь не с чем расходиться, а сверка с ним была бы дырой в запрете: заметки,
