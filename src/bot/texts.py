...
TEXTS: dict[str, dict[str, str]] = {
    ...
    # Команда вместо названия точки (T250, issue #203). До этой задачи она
    # молча становилась названием и уезжала в шапку отчёта и в имя файла.
    # Сказать обязательно: команда выполняется, и её ответ ложится в чат
    # СЛЕДОМ за этой строкой — без неё аудитор прочитает только ответ команды и
    # решит, что мастер закончился.
    "start.unit_command": {
        "ru": (
            "Это команда, а не название точки — выполняю её. "
            "Название пиццерии жду по-прежнему: пришлите его следующим сообщением."
        ),
        "en": (
            "That is a command, not a pizzeria name — running it. "
    ...
    # Решение D064 сняло кнопку подтверждения с текстового пути; добавление —
    # задел на будущее.
    #
    ...
    # формулировка — читается. Риск в том, что «кассовая зона, просрочка
    # чизкейк» напрашивается на CLN02 (пункт другой зоны) вместо верного
    # PRD10, и разницу видно только по
    ...
    # Тот же показ без строки «в отчёт»: формулировки у записи нет (так ложится
    # ручной выбор пункта по кадру без комментария — `_save_manual`, T287, D128)
    # либо она дословно совпала с вопросом пункта. Пустые кавычки читались бы
    # сбоем бота, а повтор одного и того же — двумя разными вещами.
    ...
    # Расхождение названной зоны со словарём объектов (T266). Обе стороны
    # названы дословно: аудитор обязан увидеть, ЧТО с чем разошлось, иначе
    # выбор из двух кнопок для него — гадание. Молча не пишется ни то, ни
    # другое: случай владельца — «холодный цех, пицца-печь».
    "record.zone_conflict": {
        "ru": (
            "Вы назвали зону «{spoken}», а «{object}» по карте кадров стоит "
            "в зоне «{dictionary}». Так не бывает: или речь об этом объекте, "
            "и зона у него своя, или это другой пункт."
        ),
        "en": (
            "You named the “{spoken}” zone, but the photo cue map keeps “{object}” "
            "in “{dictionary}”. Both cannot be true: either this is that object, "
            "with its own zone, or this is a different item."
        ),
    },
    # Зона спрашивается ПОСЛЕ выбора пункта (T266): кнопки тогда — только зоны,
    # допустимые методикой для этого пункта, и нажатие уже не кончится отказом
    # движка. Код пункта в тексте, потому что список кнопок собран под него.
    "record.ask_zone_for_item": {
        "ru": "В какой зоне это? {code} методика держит в зонах ниже — назовите зону кнопкой.",
        "en": "Which zone is this? The methodology keeps {code} in the zones below — pick one.",
    },
    # Слово ничего не подняло, и перечень обязан остаться доступным: тупика нет
    # ни в одной ветке (#219). Листание всех пунктов — запасной вход, и текст
    # говорит об этом прямо, чтобы аудитор не считал его единственным.
    "record.nothing_by_words": {
        "ru": ("По вашим словам пункт не нашёлся. Назовите зону — покажу её пункты списком."),
        "en": ("Your words match no item. Name the zone and I will list the items it holds."),
    },
    ...
    # Зона взята из КАРТЫ КАДРОВ: аудитор её не называл, а управляющая компания
    # зафиксировала за этим объектом одну зону и никакой другой (T262, T263).
    # На месте этого текста до T264 стояла оговорка про ПРОШЛУЮ запись, и она же
    # была признаком дефекта #218: зона прошлой записи подставлялась молча, и
    # пункт про печь уезжал в холодный цех. Прошлая запись источником быть
    # перестала, а сказать, откуда зона, по-прежнему надо: подтверждать запись
    # аудитор не будет.
    "record.fixed_zone_from_cues": {
        "ru": (
            "\n⚠ Зону вы не называли — взял её из карты кадров по названному "
            "объекту. Не та — кнопка «Зона»."
        ),
        "en": (
            "\n⚠ You named no zone — I took it from the photo cue map by the object "
            "you named. Wrong? Use “Zone”."
        ),
    },
    # Зона взята из ПУНКТА, а не из памяти (#218). Отдельный текст, потому что
    # это другой факт: методика держит этот пункт в одной зоне, и подставлять
    # прошлую было бы не догадкой, а ошибкой. Молчать всё равно нельзя — зону
    # аудитор не называл, а подтверждать запись он не будет.
    "record.fixed_zone_from_item": {
        "ru": (
            "\n⚠ Зону вы не называли — поставил ту, в которой этот пункт "
            "и бывает. Не та — кнопка «Зона»."
        ),
        "en": (
            "\n⚠ You named no zone — I used the only one this item lives in. Wrong? Use “Zone”."
        ),
    },
    "record.zone_unusual": {
    ...
    # помнить, что именно бот только что сказал двумя строками выше.
    "btn.cues_zone": {"ru": "Зона объекта: {zone}", "en": "The object’s zone: {zone}"},
    "btn.cues_item": {"ru": "Другой пункт", "en": "A different item"},
    ...
    # --- установка MCP: готовая команда для терминала (T209 → T253 → T261 →
    #     #222, решения D087, D098, D099, D103) ---
    #
    # Пункт устроен как в соседнем продукте: в чат приходит готовая команда,
    ...
    # него самого (`src/db/mcp_access.py`), поэтому команда приходит готовой
    ...
    # нельзя.
    #
    # T261 добавил вопрос «какой Claude», #222 его снял: поддерживается
    # Claude Desktop, и команда у него одна. Сама команда — в
    # `src/bot/mcp_setup.py`, здесь только то, что человек читает вокруг неё.
    # --- одна кнопка, а не развилка «какой Claude» (T261 → #222, решение D103) ---
    #
    # Развилку владелец снял прямо: поддерживается Claude Desktop, и спрашивать
    # тут не о чем. Прежде пункт спрашивал, потому что команды у приложения и у
    # терминала разные; теперь у продукта одна поддерживаемая настройка, и
    # вопрос стал лишним выбором на ровном месте.
    #
    # Кнопка при этом ОСТАЛАСЬ, и это не остаток развилки. Токен выпускается по
    # нажатию, а выпуск гасит прежний — то есть ломает уже работающую настройку.
    # Открыть пункт и посмотреть не должно ничего ломать: до T261 так и было, и
    # человек узнавал об этом из третьего сообщения. Кнопка одна, и на ней
    ...
    # через месяц, читает её отдельно от объяснения над ней.
    "btn.mcp_send": {"ru": "Прислать команду настройки", "en": "Send the setup command"},
    "mcp.setup": {
        "ru": (
    ...
    # литеральная скобка ломает подстановку. Удваивать их пришлось бы во всём
    # теле команды — и строка в исходнике перестала бы совпадать с тем, что
    # человек вставит в терминал, то есть сверить её глазами стало бы нечем.
    #
    # Что там происходит по существу (#222, решения D055 и D103): команда
    # кладёт на машину человека мост stdio↔HTTP из `tools/mcp_bridge.sh` и
    # дописывает одну запись в файл настроек приложения штатным `plutil`.
    # Ни Node, ни Python на его машине для этого не нужны.
    #
    # Это сообщение печатает в терминале сама команда, когда всё удалось.
    # Отдельным ключом — потому что оно на языке человека, как и остальной
    # разговор, а не на языке того, кто писал скрипт.
    "mcp.desktop_done": {
        "ru": "dodo-audit подключён, перезапустите Claude Desktop",
        "en": "dodo-audit connected, restart Claude Desktop",
    },
    ...
    "mcp.desktop_note": {
        "ru": (
            ...
            "кладёт обратно. Ставить для этого ничего не нужно — команда обходится тем, "
            "что на маке есть всегда."
        ),
        "en": (
            ...
            "it back. Nothing needs installing for this — the command only uses what every "
            "Mac already has."
        ),
    },
    ...
    # у тех ли доступ, кого назначил владелец (D098), а проверяется это по
    # идентификаторам.
    "mcp.who_line": {
...
def with_photo_rule(text: str, lang: str) -> str:
    ...
    Правило звучит и ЗАРАНЕЕ, и по факту намеренно (D078): человек должен
    понять, что фото обязательно, а понимает он то, что прочитал до ошибки,
    а не только то, чем ему ответили потом.
    """
