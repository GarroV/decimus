...
TEXTS: dict[str, dict[str, str]] = {
    ...
    # Отказ называет форматы, которые РАБОТАЮТ, а не один канонический (#220).
    # Прежний текст требовал «14.09.2026» и тем самым учил человека неправде:
    # дата разбирается внутри фразы и названием месяца тоже. Про год сказано
    # прямо, потому что не названный год достраивается вперёд, а человек имеет
    # право знать это до того, как срок уедет партнёру.
    "info.bad_date": {
        "ru": (
            "Не понял дату в «{text}». Дату можно написать внутри фразы — «до 15.09.2026», "
            "«созвон 15 сентября», «15.09 в 18:30». Год можно не называть: возьму ближайший, "
            "чтобы срок не оказался в прошлом. Или пропустите вопрос."
        ),
        "en": (
            "I could not read a date in “{text}”. A date may sit inside the phrase — "
            "“by 15.09.2026”, “the call is on September 15”, “15.09 at 18:30”. The year is "
            "optional: I take the nearest one that is not in the past. Or skip the question."
        ),
    },
