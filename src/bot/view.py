...
def confirmed_block(
    ...
    **код глазами не читается**. Это тот же довод, по которому вопрос пункта
    ...
    Формулировки у записи нет — строка «в отчёт» не печатается вовсе: так
    ложится ручной выбор пункта по кадру без комментария (T287, D128). Пустые
    кавычки на её месте читались бы сбоем бота, а не отсутствием слов, и это
    ровно тот показ, ради которого вопрос пункта когда-то и подставляли в текст
    записи. Пункт при этом назван словами, а «Формулировка» под записью ждёт
    настоящих слов аудитора.

    Совпали текст записи и вопрос пункта — показывается тоже одно: повтор выдал
    бы за две вещи одну. Случай остался живым и после T287 — аудитор вправе
    написать ровно то же, что стоит в методике.
    ...
    if not note.strip() or note.strip() == title.strip():
