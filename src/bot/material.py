...
class MaterialStore:
    ...
    **Ключ — чат, а срок жизни — проверка** (T249, issue #202). Разница между
    этими двумя вещами и была дефектом: очередь жила в памяти процесса и не
    кончалась вместе с проверкой, поэтому кадр НОВОЙ проверки забирал слова,
    придержанные в старой, — и фотография одной пиццерии уезжала в отчёт
    партнёру под формулировкой, сказанной в другой. Перезапуск бота дефект
    маскировал: после него в памяти пусто.

    Второго ключа (номера проверки) здесь нет намеренно: он превратил бы
    забытую очистку в тихое накопление очередей, а так лишнее удаляется, а не
    отфильтровывается. Зовёт `forget` тот же модуль и в тех же двух местах,
    что и `PendingStore.forget` (`routers/start.py`): начало новой проверки и
    «Убрать из чата».
    """
    ...
    def forget(self, chat_id: int) -> None:
        """Забыть ожидания чата: проверка кончилась, к следующей они не относятся.

        Уносится очередь целиком — и кадры без комментария, и придержанные
        слова, и постоянный индекс «сообщение → группа»: ответ на кадр прошлой
        проверки не должен заводить запись в новой. Чата, в котором очереди не
        было, это не касается — новая проверка без ожиданий обычный случай, а
        не ошибка.

        Рядом стоят ровно такие же `sidecar.reset` и `PendingStore.forget`: все
        трое — память о ТОЙ проверке, и жить обязаны ровно столько же, сколько
        она.

        **Почему очистка на СТАРТЕ новой, а не на завершении старой.** Сданную
        проверку дополнить нечем: кадр, комментарий и правка упираются в отказ
        запечатанной проверки (`sealed.py`), то есть до очереди не доходят
        вовсе. Единственный, кто мог бы забрать ожидания старой проверки, —
        новая, и очистка стоит ровно на её пороге. Та же причина и тот же
        выбор, что у `PendingStore.forget`, который живёт здесь с T206.
        """
