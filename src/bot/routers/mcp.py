...
from ..mcp_setup import setup_command
from ..texts import t
...
def build_mcp_router(settings: BotSettings) -> Router:
    ...
    async def on_send(callback: CallbackQuery) -> None:
        ...
        await message.answer(setup_command(url=setup_url(lang), token=выпущен.value))
    ...
    async def on_who(message: Message) -> None:
        ...
        Ради этого след и заведён: что доступ остаётся ровно у тех, кого
        назначил владелец (D098), через месяц нечем проверить, если этого не
        показать. Отвечает идентификаторами и датами — по ним и сверяют.
        """
        lang = chat_ui_lang(message.chat.id)
        if await _guard(message, settings) is None:
            return
