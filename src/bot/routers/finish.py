...
async def deliver(message: Message, chat_id: int, lang: str, *, allow_missing: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="bot-report-") as tmp:
        ...
        # Накопитель непокрытых формулировок уезжает в свою историю вместе с
        # проверкой (T269, #228): собранное относится к ней, а следующая
        # обязана начинаться с пустого накопителя — иначе слова прошлой точки
        # посчитались бы в этой. Перенос локальный, базы не касается, поэтому
        # стоит здесь, а не в сливе в историю (`archive`): без базы слив
        # законно не делается вовсе, а сигнал терять нельзя — ради этого
        # накопитель и лежит рядом с заметками, а не в базе.
        #
        # Отказ здесь ничего за собой не роняет по той же причине, что и
        # отметка о сдаче: отчёт уже у аудитора. Цена названа: накопитель
        # останется открытым и доедет до истории следующим завершением.
        try:
            await asyncio.to_thread(domain.archive_uncovered, chat_id)
        except (DomainError, OSError):
            logger.exception("накопитель чата %s не уехал в историю", chat_id)
        try:
