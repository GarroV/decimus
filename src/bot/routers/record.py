...
def build_record_router(*, store: MaterialStore, pending: PendingStore) -> Router:
    ...
    async def _save_manual(
        ...
        методики написаны утверждением нормы («Замороженные продукты
        размораживаются в соответствии со стандартом»), поэтому партнёр читал
        подтверждение нормы ровно там, где зафиксировано отклонение, — и читал
        как слова аудитора. Правила фиксации требуют обратного: в поле
