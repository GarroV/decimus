...
async def _show_candidates(
    ...
    *,
    ...
    if also_seen:
        # Увиденное на кадрах сверх слов (D208) — своим заголовком: аудитор
...
async def _ask_zone(
    ...
    *,
    ...
    (D206: зона — там, где продукт). До D206 здесь стояли только зоны пункта, и
...
async def _analyze_resolved(
    ...
    *,
    ...
    """
    note = base.note
    # Пачка с комментарием разбирается моделью всегда (D208): быстрый путь
...
async def _offer_also_seen(
    ...
    """Предложить увиденное на кадрах пачки сверх слов аудитора (D208).
...
def build_record_router(*, store: MaterialStore, pending: PendingStore) -> Router:
    ...
    async def on_pick(callback: CallbackQuery) -> None:
        ...
        # находки (D206): спрашивать её второй раз незачем.
