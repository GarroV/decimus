...
Задача T165, решение D077: при несостыковках между сверкой и словами аудитора,
а также когда аудитор произносит формулировку, которой в списке терминов ещё
нет, список терминов дополняется — но предложением человеку, а не
автоматически.
...
from .photo_cues import cell_codes
...
class CueRow:
    ...
    Строка хранится ЯЧЕЙКАМИ, а не одним списком кодов, и это не педантизм.
    Колонки значат разное («грязь» и «поломка» — два вопроса про один
    объект), правка принимает ровно те ячейки, которые называет карта
    (`photo_cues.editable_cells`), и плоский список дал бы предложение,
    которое `edit_photo_cue` отклонит по ширине. Хранятся ячейки словами, а не
    разобранными кодами, потому что рядом с кодами в строке стоят колонки
    управляющей компании («Откуда» — кем строка заведена): собранный из одних
    кодов вызов затёр бы их пустотой.
    """
    ...
    def columns(self) -> tuple[tuple[str, ...], ...]:
        """Коды ячеек по колонкам — тем же разбором, каким их читает правка."""
        return tuple(cell_codes(ячейка) for ячейка in self.cells)
...
def _edit(row: CueRow, suggested: str, recorded: str) -> dict[str, Any]:
    ...
    сколько их называет правка, — иначе она отклонит строку по ширине.

    Остальные ячейки переписываются КАК ЕСТЬ, а не собираются из кодов:
    рядом с кодовыми колонками в карте стоят колонки управляющей компании, и
    собранное из кодов предложение стёрло бы их содержимое (T289).
    """
    ячейки: list[str] = []
    for ячейка in row.cells:
        коды = cell_codes(ячейка)
        ячейки.append(
            f"{ячейка}, {recorded}" if suggested in коды and recorded not in коды else ячейка
        )
    return {
        "tool": "edit_photo_cue",
