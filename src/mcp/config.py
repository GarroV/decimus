...
`docs/furca/blocks/db.md`). Снаружи арендатор не принимается ни в каком виде —
...
class Settings:
    ...
    def may_read_checklist(self, tenant: str) -> bool:
        """Открыт ли ЭТОМУ арендатору ИСХОДНИК эталона — чтение, не правка.

        Право разведено с `may_manage_checklist` намеренно (T315, D132).
        Чек-лист — эталон управляющей компании, и правит его только она; но
        проверяемая сторона обязана видеть, по какому эталону её проверяют —
        владелец дословно: «базово он должен будет видеть исходник». Поэтому
        чтение не выдаётся списком: его имеет всякий заведённый арендатор, и
        единственное условие — что методика на сервере вообще настроена.

        Арендатор сверяется со списком заведённых, а не принимается на слово:
        сюда он приходит из токена, но настройки бывают собраны и мимо
        `load_settings` (ровно тесты и будущий вызывающий), и чтение, открытое
        незаведённому коду, было бы правом, выданному никому — или кому-то.
        """
        return (
...
#: Переменная связи с базой. Здесь она не строка подключения, а признак: есть
#: она — значит вторая дверь (личные токены, T253) на этом стенде включена.
DATABASE_URL_VAR = "DATABASE_URL"


def _check_any_door(tokens: Mapping[str, str], src: Mapping[str, str]) -> None:
    """Хотя бы одна дверь обязана быть открыта, иначе сервер поднялся бы открытым.
    ...
    Проверяется НАЛИЧИЕ переменной, а не связь с базой: связи здесь нет по
    устройству модуля (`load_settings` её не трогает), а стенд, у которого
    база прописана и недоступна, — это поломка стенда, и она видна в журнале
    при первой же сверке токена, а не молчаливо открытый сервер.

    Почему отказ важнее удобства именно на старте: с T255 сервер поднимается
    сервисом с `restart: unless-stopped`. Ошибка настройки, не остановленная
    здесь, превратилась бы в бесконечный перезапуск, а снаружи стенд выглядел
    бы поднятым.
    """
    if tokens or (src.get(DATABASE_URL_VAR) or "").strip():
        return
    raise McpConfigError(
        f"Ни одной двери: {MCP_TOKENS_VAR} пуста и {DATABASE_URL_VAR} не задана. Без карты "
        f"«арендатор=токен» и без базы личных токенов сервер поднялся бы открытым — за дверью "
        f"история проверок партнёра. Достаточно любой из двух: пример карты — в .env.example, "
        f"личные токены выдаёт бот пунктом «Установка MCP»"
    )
