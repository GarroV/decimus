...
def _shares(shares: object) -> str:
    """Набор долей → строка «код=доля,код=доля» для движка. Негодное — отказ здесь.

    Коды проверяются до движка по той же причине, что и у остальных правок:
    код с запятой или равенством внутри разобрался бы движком как ВТОРАЯ пара
    «код=доля», то есть опечатка в одном коде молча задала бы долю другой зоне.
    Сама арифметика долей не проверяется ни здесь, ни где-либо ещё в продукте:
    сумма, допуск и отказ живут в движке (T103, T112), и вторая их копия
    ...
    for код, значение in shares.items():
        проверенный = store_api.check_code(str(код))
        try:
            доля = float(значение)
...
def set_zone_shares(
    *,
    ...
    """Задать доли существующих зон. Названные меняются, остальные не трогаются.

    Доля — вес зоны в оценке, то есть цена ответа, а не оформление. Поэтому
    набором за один вызов: доли складываются в 100%, версию с несошедшейся
    суммой хранилище не примет (T103), и правка по одной доле до расчёта не
    дошла бы вовсе. Раздать освободившееся за управляющую компанию движок не
    вправе (T112) — расклад называет она сама.
    """
    return _change(
        store,
        tenant=tenant,
        tool="set_zone_shares",
        command="zone-share",
        options={"shares": _shares(shares)},
        version_name=version_name,
        note=note,
    )
...
def rename_zone(
    *,
    ...
    """Переименовать зону. Меняются имена, код зоны не меняется никогда.

    Кодом зона связана с колонкой `zones` у пунктов чек-листа, с записанными
    проверками и с картой слов; формулировки переводятся и правятся, коды нет.
    Названо одно имя — второе остаётся прежним; не названо ни одного — отказ,
    а не молчаливое «готово».
    """
    return _change(
        store,
        tenant=tenant,
