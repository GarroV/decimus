...
def load_state():
    ...
    with open(STATE, encoding="utf-8") as f:
        return raise_seq(json.load(f))


def raise_seq(st):
    """Счётчик номеров выводится при чтении состояния — из ВЫДАННЫХ номеров (T295).

    Номер записи аудитор называет вслух, и он уходит партнёру в письме, поэтому
    нумерация сквозная: снятая запись уносит свой номер с собой. Счётчик `seq`
    завёл `init` (T013), но в файлах старшего поколения его нет вовсе, и там
    номер выводился из максимума ЖИВЫХ записей — а `drop` этот максимум уносит.
    Проверка с записями 1-3 после снятия третьей выдавала следующей снова #3,
    и ссылка «нарушение #3» вела на другое нарушение; снятие всех записей
    возвращало нумерацию к #1.

    Поэтому счётчик поднимается один раз, при чтении: он не меньше любого уже
    выданного номера. С первой же записи состояния выведенное значение уезжает
    в файл, и дальше счётчик только растёт.
    """
    issued = [int(f["n"]) for f in st.get("findings") or []]
    st["seq"] = max([int(st.get("seq") or 0), *issued])
    return st
